...
exports.studentsendmail = catchAsyncErrors(async (req, res, next) => {
  ...
  const mailOptions = {
    from:"Master Pvt. Ltd <hi.shivamkumarshah1210@gmail.com>",
    to: req.body.email,
...
exports.applyinternship = catchAsyncErrors(async (req, res, next) => {
  ...
  const mailOptions = {
    from:"Master Pvt. Ltd <hi.shivamshah1210@gmail.com>",
    to: student.email,
...
exports.readallinternships = catchAsyncErrors(async (req, res, next) => {
  const internships = await Internship.find().populate("employe").exec();
...
exports.appliedinternship = catchAsyncErrors(async(req, res,next)=>{
  const student = await Student.findById(req.id).populate("internships").exec();
  console.log(student);
  res.status(200).json({student})
})

exports.appliedjob = catchAsyncErrors(async(req, res, next)=>{
  const student = await Student.findById(req.id).populate("jobs").exec();
  console.log(student);
  res.status(200).json({student})
})